...
::: {.try}
Change the line `format: html` in the YAML header to the following. Make sure to keep the spaces exactly the same (YAML is very picky about spaces).
...
Also, add the two commented lines below to the code chunk:
...
The `df-print: paged` option prints data frames using `rmarkdown::paged_table()` automatically. You can use `df_print: kable` to default to the simple kable style, but you will need the code from @sec-md-tables for more complex tables with kableExtra.
...
Once you're done, share your knitted html file and the qmd file for the exercises on Teams in the Week 02 channel.
