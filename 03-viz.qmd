...
```{r}
#| eval: false
pip <- read_csv("https://psyteachr.github.io/reprores-v5/data/pip_gini_welfare_2010.csv")
```
:::
...
After you update the column types, you have to re-import the data by adjusting the `read_csv()` code to set the `col_types` argument to the new column types. This is why you have to "hard code" the levels, since you won't have read in the data yet when you need to use them.
...
#| fag-alt: "The plot above with the y-axis set as a log10 scale, with breaks at 1, 2, 5, 10, 20, 50 and 100."
...
#| fig-alt: "A column plot identical to the bar plot above, except the y-axis label is n and not count. It has region_code on the x-axis with 7 categories and n on the y-axis, ranging from 0 to 30. The x-axis labels are: SAS, MNA, EAP, SSA, LAC, ECA, OHI. Bars increase from about 3 to 27."
...
#| fig-alt: "A plot with welfare_mean on the x-axis, ranging from 0 to 90, and count on the y-axis, ranging from 0 to 11. The histogram shows most values between 0 and 25, with fewer spread out between 30 and 90."
...
#| fig-alt: "A poiont plot with score (0-100) on the xi-axis and rating (1-5) on the y-axis. Since there are only 5 scores, all of the points are bunched up on the same 5 lines and span nearly the whole x-axis range. This is difficult to read."
