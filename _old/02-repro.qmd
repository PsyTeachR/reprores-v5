...
| Where | Example |
|-------|---------|
| on the web               | "https://psyteachr.github.io/reprores-v4/data/5factor.xlsx" |
...
::: {.info data-latex=""}
There is a new type of reproducible report format called [quarto](https://quarto.org/) that is very similar to R Markdown. We won't be using quarto in this class because it has a few small differences that get confusing if you're learning both quarto and R Markdown at the same time, but you should be able to pick up quarto very easily once you've learned R Markdown.
:::
...
::: {.try data-latex=""}
Create a new quarto file from the **`File > New File > R Markdown...`** menu. Change the title and author, save the file as `02-repro.Rmd`, then click the knit button to create an html file.
:::
...
When you Create a new quarto file in RStudio using the default template, a setup chunk is automatically created.
