...
Download the [Data tidying cheat sheet](https://rstudio.github.io/cheatsheets/html/tidyr.html).
...
#| fig-alt: "Two excel interfaces. The left shows a table with columns Row Labels, 2018, 2019, 2020 and Grand Total, with 5 rows, plus the pivot table builder popup interface. The right shows the same table in a longer format, with columns Row Labels and Total and a row for each year under the 5 row labels."
...
```{r}
#| label: fig-long-ggplot-demo
#| fig-cap: "Most plots are easier to make with data in a long format."
#| fig-alt: "A histogram with totalprice on the x-axis (0-60) and count on the y-0axis (0-4). The years are represented by colours: 2018 = red, 2019 = green, 2020. blue. The histogram shows "
ggplot(long_totalprice, aes(x = totalprice, fill = year)) +
  geom_histogram(binwidth = 10, color = "black", border = 0)
```
...
<img src="images/tidy/pipe_sticker.png" 
     alt="Hexagon sticker with the magrittr pipe %>% and text in cursive below it reading 'Ceci n'est pas un pipe.'" 
     style="width: 100%"/>
...
```{r}
#| webex.hide: "Alternative solution"
...
```{r}
#| label: img-exc-plot-1
#| echo: false
#| fig-alt: A scatterplot plus regression line plot titled 'Satisfaction and recommendation scores' with 'Satisfaction score' on the x-axis with ticks 1-5, 'Recommendation score' on the y-axis with ticks 1-5. The legend for 'repeat_patient' is red for no and blue for yes. The plot shows no relationship, but satistfaction scores are higher for no than yes.
...
```{r}
#| label: img-exc-plot-2
#| echo: false
#| fig-alt: A pair of boxplots with 'repeat_patient' (no / yes) on the x-axis and 'response' (1-5) on the y-axis. The left boxplot facet header is 'rec' and the right one is 'sat'. The 'no' boxplots are green and the 'yes' ones are orange. 
...
```{r}
#| label: img-exc-plot-3
#| echo: false
#| fig-alt: A histogram with the x-axis 'Responses across all questions and categories' (1-5) and y-axis 'count' (0-150) and 5 grey bars with black borders (heights 63, 57, 135, 121, and 124).
