...
Run a t-test on the UK data to see if there is a gender difference in the characteristic. Use a two-tailed test with an alpha value of 0.05. Report the outcome of the test using inline code.

### Simulation

Create a custom function that takes the means and standard deviations for male and female faces for a specified region and creates a new dataset of 30 male and 30 female faces with simulated average ratings with those distributions.
...
Run this custom function 100 times for each face number and report what percent of the runs give a significant p-value with a critical alpha of 0.05. Plot the resulting power versus face sample size.
