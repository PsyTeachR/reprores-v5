...
## Intended Learning Outcomes {#sec-ilo-data - .ilo}
...
1. Create a new quarto file called `app-data-import.qmd`
...
### New Script {#sec-exercises-new-impost}
...
Add useful labels to the columns `height`, `weight`, and `gender` for use in plotting below. For example, `height` would be best on a plot as `Height (cm)`.

```{r}
#| webex-hide: true

attr(starwars$height) <- "Height (cm)"
attr(starwars$mass) <- "Weight (kg)"
attr(starwars$gender) <- "Gender of character"
...
#| echo: false
#| warning: false
#| fig-alt: "A histogram with the title 'Height (cm) distribution of Star Wars Characters. The x-axis 'Height (cm)' runs from 75 to 275 by 25, and the y-axis 'count' runs from 0 to 40 by 10. Bars are grey with black borders and 25 units wide, centered on each tick label. Most bars are below 10, except 175 (almost 40) and 200 (about 20)."
...
#| echo: false
#| warning: false
#| fig-alt: "A scatterplot with the title 'Mass (kg) by height (cm) distribution of Star Wars Characters'. It has x-axis 'Height (cm)' with labels 0 to 300 by 50s. It has y-axis 'Mass (kg)' with labels 0 to 1400 by 100s. There are 59 solid circle points, mostly at mass below 200, with one outlier at mass 1350 and height 175."
...
```{r}
#| echo: false
#| fig-alt: "A bar chart titled 'Number of Star Wars characters of each gender'. It has x-axis 'Gender of character' with labels Masculine, Feminine and Missing. It has y-axis 'count' with labels 0 to 60 by 20s. There are 3 vertical bars. Feminine = 17, masculine = 66, missing = 4."
ggplot(starwars, aes(x = gender, fill = gender)) +
  geom_bar(show.legend = FALSE, colour = "black") +
  scale_x_discrete(labels = c("Feminine", "Masculine", "Missing")) +
...
ggplot(starwars, aes(x = gender, fill = gender)) +
  geom_bar(show.legend = FALSE, colour = "black") +
  scale_x_discrete(labels = (c("Masculine", "Feminine", "Missing"))) +
  scale_fill_brewer(palette = 2) +
  labs(title = "Number of Star Wars characters of each gender") +
  theme_bw()
```
